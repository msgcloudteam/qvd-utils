...
mod qvd {
    ...
    #[pyfunction]
    fn transform_qvd_to_parquet(py: Python,
                                source_file_name: String,
                                target_file_name: String) -> PyResult<()> {
        let xml: String = get_xml_data(&source_file_name).map_err(|e| PyErr::new::<pyo3::exceptions::PyIOError, _>(e.to_string()))?;
        let binary_section_offset = xml.as_bytes().len();
        let qvd_structure: QvdTableHeader = from_str(&xml).map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;

        let mut columns_data: Vec<(String, Vec<Option<String>>)> = Vec::new();
        ...
        if let Ok(f) = File::open(&source_file_name) {
            ...
            for field in qvd_structure.fields.headers {
                ...
                let symbol_indexes = get_row_indexes(&rows_section, &field, record_byte_size);
                let column_values = match_symbols_with_indexes(&symbols, &symbol_indexes);
                columns_data.push((field.field_name, column_values));
            }
        } else {
            return Err(PyErr::new::<pyo3::exceptions::PyIOError, _>(format!("Could not open file: {}", source_file_name)));
        }

        // Create Parquet Schema
        let mut fields: Vec<Arc<Type>> = Vec::new();
        for (name, _) in &columns_data {
            let field_type = Type::primitive_type_builder(name, PhysicalType::BYTE_ARRAY)
        ...
        let file: File = File::create(&target_file_name).map_err(|e| PyErr::new::<pyo3::exceptions::PyIOError, _>(e.to_string()))?;
        let props = Arc::new(WriterProperties::builder().build());
        let mut writer: SerializedFileWriter<File> = SerializedFileWriter::new(file, schema_ptr, props).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;

        // Write Data
        let mut row_group_writer: SerializedRowGroupWriter<_> = writer.next_row_group().map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;

        for (_, col_values) in columns_data {
            let ser_writer: Option<SerializedColumnWriter> = row_group_writer.next_column().map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
            if let Some(mut col_writer) = ser_writer {
                if let ColumnWriter::ByteArrayColumnWriter(ref mut typed_writer) = col_writer.untyped() {
                    let mut values = Vec::with_capacity(col_values.len());
                    let mut def_levels = Vec::with_capacity(col_values.len());
                    ...
                    typed_writer.write_batch(&values, Some(&def_levels), None).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
                }
            }
        }

        row_group_writer.close().expect("TODO: panic message");
